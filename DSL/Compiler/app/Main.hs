...
assignParser = Assign <$> (many1 letter) <*> (many1 space >> char '=' >> many1 space >> number)
...
whileParser = While <$> (string "while" >> many1 space >> many1 letter) <*> (many1 space >> string "positive" >> many1 space >> char '{' >> spaces >> many (statementParser <* spaces) <* char '}')
...
statementVariables (Assign v _) = [v]
